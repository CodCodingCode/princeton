...
[backend/CLAUDE.md](backend/CLAUDE.md) has a deeper architecture reference but parts of it still describe the older Streamlit + VCF + melanoma-only orchestrator design - trust the code over that file when they disagree. The pipeline is now broadly oncology, not melanoma-only.
